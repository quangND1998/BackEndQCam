...
<template>
  <div class="relative">
    <div v-if="visible" class="w-96 rounded-lg bg-[#FCFBB4] pb-1 shadow-lg absolute -top-[340px]">
      ...
      <textarea
        :value="noteForm.note"
        @input="onChangeNote"
        class="w-96 resize-none bg-[#FCFBB4] text-black border-none text-sm focus:!outline-none focus:!border-none focus:!shadow-none focus:ring-0" rows="10"></textarea>
      <div class="flex invisible justify-end py-1" :class="{
        '!visible': isLoading,
      }">
        <SpinnerIcon class="text-[#FF6100]" />
      </div>
    </div>
