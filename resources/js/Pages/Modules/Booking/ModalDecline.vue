        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <div class="flex">
                        <div class="ml-2">
                            <h3 class="text-black font-bold my-1 text-base">Xác nhận lý do hủy mã booking {{ form.ballot_code }}</h3>
                        </div>
                    </div>
                    ...
                    <form @submit.prevent="orderCancel">
                        ...
                        <div class="form-group" :class="form.errors.reason ? 'has-error' : ''">
                            <textarea v-model="form.reason" id="message" rows="4"
                                ...
                                placeholder="Tại sao khách muốn hủy mã booking?"></textarea>
...
onMounted(() => {
    emitter.on('OpenModalDecline', (booking_history, index, ref_id) => {
        // console.log(order)

        form.id = booking_history.id,
        form.ballot_code = booking_history.ballot_code
    })
});
const orderCancel = () => {
    form.post(route("admin.booking.changeStatus", form.id), {
        preserveState: true,
        onError: errors => {
