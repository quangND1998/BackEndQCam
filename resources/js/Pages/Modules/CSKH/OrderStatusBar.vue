...
const totalOrder = (status) => {
    var findStatus = props.statusGroup.find(e => e[props.state] == status);
    ...
    <div class="my-3">
        ...
        <div
            ...
            <Link v-if="hasAnyPermission(['order-pending'])" :href="route('admin.cskh.all')"
                ...
                :class="{ 'bg-white  text-red-500': $page.url.includes('/admin/cskh/all') }">
            <Icon icon="queue"></Icon>

            <span class="text-gray-400 ml-1" :class="{ 'bg-white  text-red-500': $page.url.includes('/admin/cskh/all') }">
                Tất cả({{ count_orders }})</span>
            </Link>

            <Link v-if="hasAnyPermission(['order-pending'])" :href="route('admin.cskh.pending')" @click.prevent
                class="min-[320px]:my-2 text-sm px-3 py-2  mx-1 bg-gray-100 hover:bg-white text-gray-500 flex flex-wrap mr-2"
                :class="{ 'bg-white  text-red-500': $page.url.includes('/admin/cskh/pending') }">
            <Icon icon="loading"></Icon>


            <span class="text-gray-400 ml-2"
                :class="{ 'bg-white  text-red-500': $page.url.includes('/admin/cskh/pending') }"> Đơn chờ({{
                    totalOrder('pending') }})</span>
            </Link>

            <Link v-if="hasAnyPermission(['order-packing'])" :href="route('admin.cskh.packing')"
                class="min-[320px]:my-2 text-sm px-3 py-2  mx-1 bg-gray-100 hover:bg-white text-gray-500 flex flex-wrap mr-2"
                :class="{ 'bg-white  text-red-500': $page.url.includes('/admin/cskh/packing') }">
            <Icon icon="packing"></Icon>
            <span class="text-gray-400 ml-2"
                :class="{ 'bg-white  text-red-500': $page.url.includes('/admin/cskh/packing') }">Đóng gói ({{
            ...
            <span class="text-gray-400 ml-2"
                :class="{ 'bg-white  text-red-500': $page.url.includes('/admin/cskh/shipping') }"> Đang vận chuyển({{
                    totalOrder('shipping') }})</span>
            </Link>
            ...
            <span class="text-gray-400 ml-2"
                :class="{ 'bg-white  text-red-500': $page.url.includes('/admin/cskh/completed') }"> Đã giao ({{
            ...
            <span class="text-gray-400 ml-2"
                :class="{ 'bg-white  text-red-500': $page.component == 'Modules/CSKH/Refunding' }"> Chờ hoàn ({{
                    totalOrder('refunding') }})</span>
            </Link>
            <Link v-if="hasAnyPermission(['order-refund'])" :href="route('admin.cskh.refund')"
                class="min-[320px]:my-2 text-sm px-3 py-2  mx-1 bg-gray-100 hover:bg-white text-gray-500 flex flex-wrap mr-2"
                :class="{ 'bg-white  text-red-500': $page.component == 'Modules/CSKH/Refund' }">

            <Icon icon="back"></Icon>
            <span class="text-gray-400 ml-2"
                :class="{ 'bg-white  text-red-500': $page.component == 'Modules/CSKH/Refund' }"> Hoàn đơn ({{
            ...
            <span class="text-gray-400 ml-2"
                :class="{ 'bg-white text-red-500': $page.url.includes('/admin/cskh/decline') }">Hủy giao ({{
