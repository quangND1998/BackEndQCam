<template>
    <div class="flex justify-between">
        <button v-if="status == 'pending' || status == 'packing'"
            class="border text-red-700 rounded-lg bg-gray-100 px-3 py-2" data-toggle="modal"
            data-target="#exampleModalDecline" @click="openDecline(order)">Hủy đơn</button>
        <button v-if="status == 'shipping' || status == 'completed'" class="border rounded-lg bg-gray-100 px-3 py-2"
            data-toggle="modal" data-target="#exampleModalRefund" @click="openRefund(order)">Hoàn đơn</button>

        <button v-if="status == 'decline'" @click="orderChangePending(order)"
            class="border border-gray-800 bg-gray-100 px-3 py-2 text-gray-900 text-sm rounded-md">Làm mới đơn
            hàng</button>
        <div class="flex">
...
const orderChangePacking = () => {
    ...
    swal
        .fire({
            title: "Bạn có muốn?",
            text: `Chuyển trạng thái đơn hàng ${props.order.order_number} sang đóng gói!`,
            icon: "warning",
            showCancelButton: true,
            confirmButtonColor: "#3085d6",
            cancelButtonColor: "#d33",
