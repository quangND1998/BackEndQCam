...
class CustomerProductOwerController extends Base2Controller
{
   ...
   public function getOneProductActivity($id)
   {
        $customer = Auth::user();
        if($customer){
        $product_owner = ProductServiceOwner::with('product.images','trees','tree.image','history_extend.contract.lastcontract.images','extend_last.contract.lastcontract.images','visit','history_gift')->where('user_id',$customer->id)->find($id);
        $dt = Carbon::now();
